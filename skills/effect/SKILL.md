...
name: effect
description: |
  Opinionated guide for building and refactoring production TypeScript applications with Effect v4. Use when implementing Effect workflows, services, layers, schemas, configuration, schedules, caches, streams, HTTP clients, tests, or functional transformations with Effect standard-library modules.
license: MIT
compatibility: Requires Effect v4. Examples are reviewed against the version documented in this repository.
metadata:
  source: https://github.com/kitlangton/skills/tree/main/skills/effect
  attribution: Modified derivative; see the repository NOTICE.md.
---
...
- Local data transformations, `pipe` / `flow`, optional values, tagged-union matching, collection work, sorting, predicates, or immutable nested updates: read `references/FUNCTIONAL_STYLE.md`.
- Typed failures, adapter error mapping, recovery, defects, interruption, `Cause`, or `Exit`: read `references/ERROR_HANDLING.md`.
- Resource acquisition, cleanup, finalizers, scopes, pools, or refreshable resources: read `references/RESOURCES_SCOPES.md`.
- Logs, log annotations, metrics, spans, tracing, OpenTelemetry, or OTLP: read `references/OBSERVABILITY.md`.
- Dates, time zones, formatting, calendar arithmetic, or current time: read `references/DATETIME.md`.
- Incoming schema-first HTTP APIs, `HttpApi`, handlers, middleware, OpenAPI, or typed API clients: read `references/HTTP_API.md`.
...
- Prefer the version-pinned unstable Effect HTTP client modules for outgoing HTTP in Effect applications when their typed errors, layers, and client transforms are useful.
...
- Prefer Effect standard-library modules for non-trivial local transformations; use direct TypeScript when it is clearer.
- Use `Option` for potentially missing collection or record access, and handle both cases at the meaningful boundary.
- Use `Match` for non-trivial branching on tagged unions or refined shapes.
- Use `Optic` for composable immutable updates through nested optional structures; use direct object updates when the path is short and clear.
...
- HTTP request in an Effect application: prefer the version-pinned unstable Effect `HttpClient` plus request/response schema decoding.
